...
export default class TrackingDetails extends React.Component {
    ...
    render() {
        let { navigation } = this.props;
        let { report_data, vehicle, device, deviceDetails, refreshing, shorting } = this.state;
        let devices = navigation.getParam("devices", null);
        // console.log('device1111', device);
        // console.log('report_data111', report_data);
        return (
            <View style={{ backgroundColor: 'white', flex: 1 }}>
                ...
                {deviceDetails != null &&
                    <View>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-around', marginTop: 10, paddingHorizontal: 5 }}>
                            <View style={{ justifyContent: 'center', alignItems: 'center', width: '33%' }}>
                                <CarIcon name="car" size={24} color='green' />
                                <Text style={{ fontSize: 12, color: 'black', fontWeight: 'bold', textAlign: 'center' }}>{deviceDetails.engine_on_time || "--"}</Text>
                                <Text style={{ fontSize: 12, fontWeight: 'bold' }}>Moving</Text>
                            </View>
                            <View style={{ justifyContent: 'center', alignItems: 'center', width: '33%' }}>
                                <RoadIcon name="road" size={24} />
                                <Text style={{ fontSize: 12, color: 'black', fontWeight: 'bold', textAlign: 'center' }}>{deviceDetails.total_distance || "--"} KMS</Text>
                                <Text style={{ fontSize: 12, fontWeight: 'bold' }}>Travelled</Text>
                            </View>

                            <View style={{ justifyContent: 'center', alignItems: 'center', width: '33%' }}>
                                <BedIcon name="bed" size={24} color='red' />
                                <Text style={{ fontSize: 12, color: 'black', fontWeight: 'bold', textAlign: 'center' }}>{deviceDetails.stoppage_time || "--"}</Text>
                                <Text style={{ fontSize: 12, fontWeight: 'bold' }}>Stoppage</Text>
                            </View>
                        </View>
                        <View style={{ flexDirection: 'row', marginTop: 10, marginLeft: 10 }}>
                            <LocationIcon name="location-pin" size={30} color="#00ace6" />
                            <View>
                                <Text style={{ fontSize: 14 }}>{device.address}</Text>
                                <Text style={{ fontSize: 14, color: 'gray' }}>Last updated {moment(device.updated_at).format('DD MMMM YYYY hh:mm: A')}</Text>
                            </View>
                        </View>
                    </View>
                }
                ...
                <View style={{ justifyContent: 'center', alignItems: 'center', marginTop: 20 }}>
                    <View style={styles.subBox}>
                        <Text style={{ fontSize: 17 }}>TRIP HISTORY</Text>
                    </View>
                </View>
                        ...
                        renderItem={({ item, index }) => {
                            return (
                                <View
                                    ...
                                    <TouchableOpacity activeOpacity={0.70} onPress={() => NavigationService.navigate('homeStack', 'MapView', { device: device, record: item, type: "idle", screen: "Idle" })}
                                        style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                        <View style={{ flexDirection: 'row' }}>
                                            <ClockIcon name="clockcircleo" size={24} color="#ff8c1a" />
                                            <Text style={{ color: '#00ace6', fontSize: 16, fontWeight: 'bold', marginLeft: 20 }}>Idle {item.id}</Text>
                                        </View>

                                        <View style={{ flexDirection: 'row' }}>
                                            <Text style={{ color: '#00ace6', fontSize: 16, fontWeight: 'bold', marginRight: 5 }}>{item.idle_time}</Text>
                                    ...
                                    <View style={styles.box2}>
                                        ...
                                        {/* <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                                <CarIcon name="car" size={24} color='gray' />
                                                <View style={{ marginLeft: 20 }}>
                                                    <Text style={{ color: 'gray' }}>Driving</Text>
                                                    <Text style={{ color: 'gray' }}>20 Min.5 Kms</Text>
                                                </View>
                                            </View> */}
                                    </View>
                                </View>
